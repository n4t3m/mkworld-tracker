...
scripts/
├── audit_track_select.py       # Regression audit: run TrackSelectDetector over every saved frame in matches/
...
## Detection Patterns
- **Track selection screen**: the track-name banner moves with the selected island (it can appear anywhere on the right-side map), and the player-list panel on the left is not always dark — team-mode matches fill it with bright coloured bars and 24-player matches widen the panel beyond the old 42% boundary. OCR sparse-scans the right ~60% of the frame at 2x scale across PSM modes 11/6/7, joins adjacent fragments (Tesseract sometimes splits the banner mid-word, e.g. "Wario S" + "hipyard"), and fuzzy-matches every line against the 30 canonical track names with `difflib` (cutoff 0.85). Fuzzy matching alone is not a sufficient screen discriminator — both the **voting screen** (3-5 candidate banners over the world map) and the **in-race track-name overlay** (3D scenery with the selected track in the top-right) hit the OCR/canonical-list path. `has_player_panel(frame)` is therefore a hard pre-check inside `detect()`: it samples a vertical strip at x=5-30%, mean-collapses to a 1D row-brightness profile, detrends with a gaussian blur (kernel=51), and computes the FFT autocorrelation peak in the lag range 40-180 px. Real track-select fixtures (no-team / team-mode / 24p) score 0.43-0.92 because the pill-row pattern is strongly periodic; voting and gameplay frames score ≤0.20. Threshold is 0.30. The state machine guards calls by game state and applies a 15s cooldown after a match. The detector is regression-audited via `scripts/audit_track_select.py` (see Scripts section), which runs `detect()` over every saved frame in `matches/` and reports any bucket-vs-detection mismatch — it caught the original false-positive pattern (voting/gameplay frames being saved as `track.png` for every race in `20260506_191354`) and is the canonical way to validate sensitivity changes.
...
- **Three async Gemini integrations** (all fire-and-forget on daemon threads):
  ...
  2. **Race results** (`gemini_results.py`): multiple scrolling result frames → structured JSON with teams, mode, and per-player placements. Stored in `RaceInfo.placements` and `RaceInfo.gemini_results`. The state machine, the regenerate-placements UI thread, and the backfill script all pass the authoritative `MatchSettings.teams` value (`"No Teams"` / `"Two Teams"` / `"Three Teams"` / `"Four Teams"`) as the `teams_setting` kwarg; `_build_prompt` folds it into the prompt as a hard "set mode=… and split into exactly N teams of 3" constraint so Gemini doesn't have to guess team mode from bar colours alone — a single race with washed-out bar colours used to come back as `no_teams` even when the match was Four Teams.
...
- All three modules strip markdown code fences from responses — both leading (` ```json … ``` `) and stray trailing-only fences (Gemini occasionally emits raw JSON followed by a hallucinated closing ``` ` `` ` ` `` `, which used to break the parser) — and retry parsing after stripping.
...
## Track-Select Detection Audit
- `scripts/audit_track_select.py` is the regression-audit tool for `TrackSelectDetector`. It walks every `*.png` under `matches/`, runs `TrackSelectDetector().detect(frame)` on each (a fresh detector per frame so the 15s cooldown doesn't suppress later frames), and groups results by basename bucket (`track.png`, `players.png`, `vote.png`, `placement_NN.png`, `debug_placements/{pre,post}_NN.png`, etc.).
- For each bucket it reports detection counts, then flags three categories:
  - **Suspect positives** — non-track / non-players frames that detected as a track. Most are expected to live in `debug_votes/vote_12.png` (the buffer-tail frame is captured at-or-near the actual track-detection moment, so it legitimately shows the track-select screen). Detections in any other bucket (`vote.png`, `finish.png`, `placement_*.png`, `rank.png`, etc.) are real false positives and require investigation.
  - **Suspect negatives** — `track.png` / `players.png` that did NOT detect. Common cause is a panel-fade transition frame captured by the OLD detector before the player panel fully rendered; the cross-check is whether the same race's `players.png` (saved one tick later) does detect — if so, the new detector would just wait one more poll cycle in production. Counts that match this pattern are not regressions.
  - **Track mismatches** — bucket detected but returned a different canonical name than `match.json`. The standing offender is `Wario Shipyard` ↔ `Wario Stadium` (same canonical-list edit-distance issue documented by the `wario_shipyard.png` regression fixture).
- Invocation: `uv run python -m scripts.audit_track_select`. Requires no Gemini API key, no Tesseract config beyond what the live app uses. Run after any change to `track_select.py` thresholds, OCR configuration, or panel-detection logic, and compare the printed counts/buckets against the previous run to spot sensitivity regressions.
